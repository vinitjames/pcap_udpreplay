...
public:
  ...
  PCAPHandle(const std::string& filename){
    ...
    _pcap_t = pcap_open_offline_with_tstamp_precision(filename.c_str(),
...
bool PCAPUDPReader::read_next(){
  ...
  const u_char* _payload_handle = pcap_next(_pcap_handle->get_handle(), &pkt_hdr);
...
const u_char* PCAPUDPReader::udp_payload() const{
  return _udp_payload;
}
...
bool PCAPUDPReader::_parse_ip6_pkt(const u_char* payload){
    const struct ipv6_hdr* ipv6_header = reinterpret_cast<const struct ipv6_hdr*>(payload);
    if(ipv6_header-> != 6){
      std::cout<< "IP Version in header is not IPV6";
      return false;
    }
    if(ipv6_header->ip_p != IPPROTO_UDP){
      std::cout<< "Packet payload not of type UDP";
      return false;
    }
    return _parse_udp_pkt(payload + ipv6_header->ip_hl*4);
  }
...
bool PCAPUDPReader::_parse_udp_pkt(const u_char* payload){
  ...
  _udp_header = {udp_hdr->source, udp_hdr->dest, ntohs(udp_hdr->len), udp_hdr->check};
